...
public class ChessBoard implements Listenable<GameListener>, Serializable {
    ...
    public void placeInitialPieces() {
        if(Chinesechecker==2 || Chinesechecker==8) {
    ...
    public ChessBoardLocation best(ArrayList<ChessBoardLocation> a,ChessBoardLocation b){//判断对于任意的棋子，哪一个最合适
        ...
        for (int i=0;i<a.size();i++){
            c[i]=a.get(i).getColumn()-b.getColumn()+a.get(i).getRow()-b.getRow();
        }
        int i=max(c);
        return a.get(i);
    }
    public ChessBoardLocation bestPlus(ArrayList<ChessBoardLocation> a,ArrayList<ChessBoardLocation> b,int d){//判断每一个棋子，哪一个最合适
        int[] c=new int[a.size()];
        for (int i=0;i<a.size();i++){
            c[i]=a.get(i).getColumn()-b.get(i).getColumn()+a.get(i).getRow()-b.get(i).getRow();
        }
        int i=max(c);
        if(d==0){
            return a.get(i);
   ...
   public boolean willWin(ArrayList<ChessBoardLocation> A,Color a){
       if (Chinesechecker == 4) {
           if (a.toString().equals(Color.GREEN.toString())) {
               for (int i = 0; i < 16; i++) {
                   for (int j = 0; j < 16 && (i + j >= 5); j++) {
           ...
           if (a.toString().equals(Color.RED.toString())) {
               for (int i = 15; i > -1; i--) {
                   for (int j = 15; j > -1 && i + j <= 26; j--) {
                       if (A.contains(new ChessBoardLocation(i, j))) {
                           return false;
                       }
                   }
           ...
           if (a.toString().equals(Color.ORANGE.toString())) {
               for (int x = 0; x < 16; x++) {
                   for (int y = 0; y < 16; y++) {
                       if (x - y <= 10) {
           ...
           if (a.toString().equals(Color.BLUE.toString())) {
               for (int x = 0; x < 16; x++) {
                   for (int y = 0; y < 16; y++) {
                       if (x - y <= 10) {
